...
public enum BondDeviceAction {
    ...
    // Actions
    TurnOn("TurnOn", CHANNEL_POWER_STATE),
    // ^^ Turn device power on.
    TurnOff("TurnOff", CHANNEL_POWER_STATE),
    // ^^ Turn device power off.
    TogglePower("TogglePower", CHANNEL_POWER_STATE),
    ...
    // Actions
    SetSpeed("SetSpeed", CHANNEL_FAN_SPEED),
    ...
    // Actions
    BreezeOn("BreezeOn", CHANNEL_FAN_BREEZE_STATE),
    // ^^ Enable breeze with remembered parameters. Defaults to [50,50].
    BreezeOff("BreezeOff", CHANNEL_FAN_BREEZE_STATE),
    // ^^ Stop breeze. Fan remains on at current speed.
    SetBreeze("SetBreeze", CHANNEL_FAN_BREEZE_MEAN),
    ...
    // Actions
    SetDirection("SetDirection", CHANNEL_FAN_DIRECTION),
    // ^^ Control forward and reverse.
    ToggleDirection("ToggleDirection", CHANNEL_FAN_DIRECTION),
    // ^^ Reverse the direction of the fan.

    // State Variables
    // light: (integer) 1 = light on, 0 = light off
    // Actions
    TurnLightOn("TurnLightOn", CHANNEL_FAN_LIGHT_STATE),
    // ^^ Turn light on.
    TurnLightOff("TurnLightOff", CHANNEL_FAN_LIGHT_STATE),
    // ^^ Turn off light.
    ToggleLight("ToggleLight", CHANNEL_FAN_LIGHT_STATE),
    ...
    // Actions
    TurnUpLightOn("TurnUpLightOn", CHANNEL_FAN_UP_LIGHT_STATE),
    // ^^ Turn up light on.
    TurnDownLightOn("TurnDownLightOn", CHANNEL_FAN_DOWN_LIGHT_STATE),
    // ^^ Turn down light on.
    TurnUpLightOff("TurnUpLightOff", CHANNEL_FAN_UP_LIGHT_STATE),
    // ^^ Turn off up light.
    TurnDownLightOff("TurnDownLightOff", CHANNEL_FAN_DOWN_LIGHT_STATE),
    // ^^ Turn off down light.
    ToggleUpLight("ToggleUpLight", CHANNEL_FAN_UP_LIGHT_STATE),
    // ^^ Change up light from on to off, or off to on.
    ToggleDownLight("ToggleDownLight", CHANNEL_FAN_DOWN_LIGHT_STATE),
    ...
    // Actions
    SetUpLightBrightness("SetUpLightBrightness", CHANNEL_UP_LIGHT_BRIGHTNESS),
    // ^^ Similar to SetBrightness but only for the up light.
    SetDownLightBrightness("SetDownLightBrightness", CHANNEL_DOWN_LIGHT_BRIGHTNESS),
    // ^^ Similar to SetBrightness but only for the down light.
    IncreaseUpLightBrightness("IncreaseUpLightBrightness", CHANNEL_UP_LIGHT_BRIGHTNESS),
    ...
    DecreaseUpLightBrightness("DecreaseUpLightBrightness", CHANNEL_UP_LIGHT_BRIGHTNESS),
    ...
    // Actions
    SetFlame("SetFlame", CHANNEL_FLAME),
    ...
    // Actions
    TurnFpFanOff("TurnFpFanOff", CHANNEL_FP_FAN_STATE),
    // ^^ Turn the fireplace fan off
    TurnFpFanOn("TurnFpFanOn", CHANNEL_FP_FAN_STATE),
    // ^^ Turn the fireplace fan on, restoring the previous speed
    SetFpFan("SetFpFan", CHANNEL_FP_FAN_SPEED),
    // ^^ Sets the speed of the fireplace fan

    // State Variables
    // open: (integer) 1 = open, 0 = closed
    // Actions
    Open("Open", CHANNEL_OPEN_CLOSE),
    // ^^ Open the device.
    Close("Close", CHANNEL_OPEN_CLOSE),
    // ^^ Close the device.
    ToggleOpen("ToggleOpen", CHANNEL_OPEN_CLOSE),
    ...
    StartDimmer("StartDimmer", CHANNEL_LIGHT_BRIGHTNESS),
    // ^^ Start dimming. The Bond should time out its transmission after 30 seconds, or when the Stop action is called.
    StartUpLightDimmer("StartUpLightDimmer", CHANNEL_UP_LIGHT_BRIGHTNESS),
    // ^^ Use this and the StartDownLightDimmer instead of StartDimmer if your device has two dimmable lights.
    StartDownLightDimmer("StartDownLightDimmer", CHANNEL_DOWN_LIGHT_BRIGHTNESS),
    // ^^ The counterpart to StartUpLightDimmer
    StartIncreasingBrightness("StartIncreasingBrightness", CHANNEL_LIGHT_BRIGHTNESS),
    StartDecreasingBrightness("StartDecreasingBrightness", CHANNEL_LIGHT_BRIGHTNESS);

    private String actionId;
    private String channelId;
    // private String [] channelIds;

    private BondDeviceAction(final String actionId, final String channelId) {
