...
public enum BondDeviceAction {
    ...
    // Actions
    TurnOn("TurnOn", CHANNEL_GROUP_BASIC, CHANNEL_POWER_STATE, "Power", "Switch"),
    // ^^ Turn device power on.
    TurnOff("TurnOff", CHANNEL_GROUP_BASIC, CHANNEL_POWER_STATE, "Power", "Switch"),
    // ^^ Turn device power off.
    TogglePower("TogglePower", CHANNEL_GROUP_BASIC, CHANNEL_POWER_STATE, "Power", "Switch"),
    // ^^ Change device power from on to off, or off to on.

    // State Variables
    // timer: (integer) seconds remaining on timer, or 0 meaning no timer running
    // Actions
    SetTimer("SetTimer", CHANNEL_GROUP_BASIC, CHANNEL_TIMER, "Timer", "Number"),
    ...
    // Actions
    SetSpeed("SetSpeed", CHANNEL_GROUP_FAN, CHANNEL_FAN_SPEED, "Speed", "Number"),
    ...
    IncreaseSpeed("IncreaseSpeed", CHANNEL_GROUP_FAN, CHANNEL_FAN_SPEED, "Speed", "Number"),
    // ^^ Increase speed of fan by specified number of speeds. If the fan is off,
    // implicitly turn on the power.
    DecreaseSpeed("DecreaseSpeed", CHANNEL_GROUP_FAN, CHANNEL_FAN_SPEED, "Speed", "Number"),
    ...
    // Actions
    BreezeOn("BreezeOn", CHANNEL_GROUP_FAN, CHANNEL_FAN_BREEZE_STATE, "Breeze", "Switch"),
    // ^^ Enable breeze with remembered parameters. Defaults to [50,50].
    BreezeOff("BreezeOff", CHANNEL_GROUP_FAN, CHANNEL_FAN_BREEZE_STATE, "Breeze", "Switch"),
    // ^^ Stop breeze. Fan remains on at current speed.
    SetBreeze("SetBreeze", CHANNEL_GROUP_FAN, CHANNEL_FAN_BREEZE_MEAN, "Breeze Average Speed", "Dimmer"),
    ...
    // respectively.
    // Actions
    SetDirection("SetDirection", CHANNEL_GROUP_FAN, CHANNEL_FAN_DIRECTION, "Direction", "Switch"),
    // ^^ Control forward and reverse.
    ToggleDirection("ToggleDirection", CHANNEL_GROUP_FAN, CHANNEL_FAN_DIRECTION, "Direction", "Switch"),
    // ^^ Reverse the direction of the fan.

    // State Variables
    // light: (integer) 1 = light on, 0 = light off
    // Actions
    TurnLightOn("TurnLightOn", CHANNEL_GROUP_LIGHT, CHANNEL_LIGHT_STATE, "Light", "Switch"),
    // ^^ Turn light on.
    TurnLightOff("TurnLightOff", CHANNEL_GROUP_LIGHT, CHANNEL_LIGHT_STATE, "Light", "Switch"),
    // ^^ Turn off light.
    ToggleLight("ToggleLight", CHANNEL_GROUP_LIGHT, CHANNEL_LIGHT_STATE, "Light", "Switch"),
    ...
    // Actions
    TurnUpLightOn("TurnUpLightOn", CHANNEL_GROUP_UP_LIGHT, CHANNEL_UP_LIGHT_STATE, "Up Light", "Switch"),
    // ^^ Turn up light on.
    TurnDownLightOn("TurnDownLightOn", CHANNEL_GROUP_DOWN_LIGHT, CHANNEL_DOWN_LIGHT_STATE, "Down Light", "Switch"),
    // ^^ Turn down light on.
    TurnUpLightOff("TurnUpLightOff", CHANNEL_GROUP_UP_LIGHT, CHANNEL_UP_LIGHT_STATE, "Up Light", "Switch"),
    // ^^ Turn off up light.
    TurnDownLightOff("TurnDownLightOff", CHANNEL_GROUP_DOWN_LIGHT, CHANNEL_DOWN_LIGHT_STATE, "Down Light", "Switch"),
    // ^^ Turn off down light.
    ToggleUpLight("ToggleUpLight", CHANNEL_GROUP_UP_LIGHT, CHANNEL_UP_LIGHT_STATE, "Up Light", "Switch"),
    // ^^ Change up light from on to off, or off to on.
    ToggleDownLight("ToggleDownLight", CHANNEL_GROUP_DOWN_LIGHT, CHANNEL_DOWN_LIGHT_STATE, "Down Light", "Switch"),
    ...
    // Actions
    SetBrightness("SetBrightness", CHANNEL_GROUP_LIGHT, CHANNEL_LIGHT_BRIGHTNESS, "Light Brightness", "Dimmer"),
    // ^^ Set the brightness of the light to specified percentage. Value of 0 is
    // ignored, use TurnLightOff instead.
    IncreaseBrightness("IncreaseBrightness", CHANNEL_GROUP_LIGHT, CHANNEL_LIGHT_BRIGHTNESS, "Light Brightness",
            "Dimmer"),
    // ^^ Increase brightness of light by specified percentage. If light is off, it
    // will be turned on at (0 + amount).
    DecreaseBrightness("DecreaseBrightness", CHANNEL_GROUP_LIGHT, CHANNEL_LIGHT_BRIGHTNESS, "Light Brightness",
            "Dimmer"),
    ...
    // Actions
    SetFlame("SetFlame", CHANNEL_GROUP_FIREPLACE, CHANNEL_FLAME, "Flame Level", "Dimmer"),
    ...
    IncreaseFlame("IncreaseFlame", CHANNEL_GROUP_FIREPLACE, CHANNEL_FLAME, "Flame Level", "Dimmer"),
    // ^^ Increase flame level of fireplace by specified number of flames. If the
    // fireplace is off, implicitly turn on
    // the power.
    DecreaseFlame("DecreaseFlame", CHANNEL_GROUP_FIREPLACE, CHANNEL_FLAME, "Flame Level", "Dimmer"),
    ...
    // Actions
    TurnFpFanOff("TurnFpFanOff", CHANNEL_GROUP_FIREPLACE, CHANNEL_FP_FAN_STATE, "Fireplace Fan", "Switch"),
    // ^^ Turn the fireplace fan off
    TurnFpFanOn("TurnFpFanOn", CHANNEL_GROUP_FIREPLACE, CHANNEL_FP_FAN_STATE, "Fireplace Fan", "Switch"),
    // ^^ Turn the fireplace fan on, restoring the previous speed
    SetFpFan("SetFpFan", CHANNEL_GROUP_FIREPLACE, CHANNEL_FP_FAN_SPEED, "Fireplace Fan Speed", "Dimmer"),
    // ^^ Sets the speed of the fireplace fan

    // State Variables
    // open: (integer) 1 = open, 0 = closed
    // Actions
    Open("Open", CHANNEL_GROUP_SHADES, CHANNEL_OPEN_CLOSE, "Shade Level", "Switch"),
    // ^^ Open the device.
    Close("Close", CHANNEL_GROUP_SHADES, CHANNEL_OPEN_CLOSE, "Shade Level", "Switch"),
    // ^^ Close the device.
    ToggleOpen("ToggleOpen", CHANNEL_GROUP_SHADES, CHANNEL_OPEN_CLOSE, "Shade Level", "Switch"),
    // ^^ Close the device if it's open, open it if it's closed

    // Other actions
    Stop("Stop", CHANNEL_GROUP_BASIC, "", "", ""),
    // ^^ This action tells the Bond to stop any in-progress transmission and empty
    // its transmission queue.
    Hold("Hold", CHANNEL_GROUP_BASIC, "", "", ""),
    ...
    Pair("Pair", CHANNEL_GROUP_BASIC, "", "", ""),
    // ^^ Used in devices that need to be paired with a receiver.
    StartDimmer("StartDimmer", CHANNEL_GROUP_LIGHT, CHANNEL_LIGHT_UNIDIRECTIONAL, "Light Brightness", "Switch"),
    // ^^ Start dimming. The Bond should time out its transmission after 30 seconds,
    // or when the Stop action is called.
    StartUpLightDimmer("StartUpLightDimmer", CHANNEL_GROUP_UP_LIGHT, CHANNEL_UP_LIGHT_UNIDIRECTIONAL,
            "Up Light Brightness", "Switch"),
    // ^^ Use this and the StartDownLightDimmer instead of StartDimmer if your
    // device has two dimmable lights.
    StartDownLightDimmer("StartDownLightDimmer", CHANNEL_GROUP_DOWN_LIGHT, CHANNEL_DOWN_LIGHT_UNIDIRECTIONAL,
            "Down Light Brightness", "Switch"),
    // ^^ The counterpart to StartUpLightDimmer
    StartIncreasingBrightness("StartIncreasingBrightness", CHANNEL_GROUP_LIGHT, CHANNEL_LIGHT_BIDIRECTIONAL,
            "Light Brightness", "Switch"),
    StartDecreasingBrightness("StartDecreasingBrightness", CHANNEL_GROUP_LIGHT, CHANNEL_LIGHT_BIDIRECTIONAL,
            "Light Brightness", "Switch"),

    // More actions
    OEMRandom("OEMRandom", CHANNEL_GROUP_BASIC, "", "", ""),
    OEMTimer("OEMTimer", CHANNEL_GROUP_BASIC, "", "", ""),
    Unknown("Unknown", CHANNEL_GROUP_BASIC, "", "", "");
    ...
    private BondDeviceAction(final String actionId, ChannelGroupTypeUID channelGroupTypeUID, String channelId,
            String channelLabel, String acceptedItemType) {
    ...
    /**
     * @return the channelGroupTypeUID
     */
    public ChannelGroupTypeUID getChannelGroupTypeUID() {
